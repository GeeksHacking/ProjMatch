...
export default function SettingsPage() {
	...
	const handleSubmit = (e) => {
		...
		var updatedData = Object.keys(userData).reduce(function (filtered, key) {
            if (key === "userDat") {
                var fileredUserDat = Object.keys(userData.userDat).reduce(function (filtered, key) {
			...
			content: (
				...
				>
					...
					<input
						type="text"
						name="rlName"
						defaultValue={Object.keys(userData).length !== 0 ? userData.rlName : ""}
						onChange={(e) => handleValueChange(e.target.name, e.target.value)}
					...
					<div className="flex h-fit w-[70%] flex-row justify-between">
						<input
							type="text"
							name="regEmail"
							defaultValue={Object.keys(userData).length !== 0 ? userData.regEmail : ""}
							onChange={(e) => handleValueChange(e.target.name, e.target.value)}
					...
					<input
						type="text"
						name="contactLink"
						defaultValue={Object.keys(userData).length !== 0 ? userData.contactLink : ""}
						onChange={(e) => handleValueChange(e.target.name, e.target.value)}
					...
					<textarea
						name="aboutMe"
						defaultValue={Object.keys(userData).length !== 0 ? userData.aboutMe : ""}
						onChange={(e) => handleValueChange(e.target.name, e.target.value)}
					...
					<div className="flex h-36 w-[70%] flex-row items-center justify-start">
						{Object.keys(userData).length !== 0 && userData.userDat.profilePic !== "" ? (
							<img
								src={
					...
					<div className="flex h-fit w-[70%] flex-col items-start justify-center">
						{Object.keys(userData).length !== 0 && userData.userDat.profileBanner !== "" ? (
							<img
								src={
		...
		},
		{
			...
			content: (
				<div className="flex w-full flex-col items-start justify-start">
					...
					(<Switch
						checked={userData.settings.web_settings.theme === "dark" ? true : false}
						onChange={() => { 
                            handleValueChange("settings", {
                                ...userData.settings,
                                web_settings: {
                                    theme: userData.settings.web_settings.theme === "dark" ? "light" : "dark"
                                }
                            })
                        }}
						className={`${
							userData.settings.web_settings.theme === "dark" ? `bg-gray-400` : `bg-gray-400`
						} relative inline-flex h-8 w-16 items-center rounded-full transition`}
					>
						<span
							className={`${
                                userData.settings.web_settings.theme === "dark" ? "translate-x-9 bg-black" : "translate-x-1 bg-white"
							} inline-block flex h-6 w-6 transform items-center justify-center rounded-full transition`}
						>
							<img
								className="h-1/2"
								src={userData.settings.web_settings.theme === "dark" ? "/IconsMoon.svg" : "/IconsSun.svg"}
							/>
						</span>
					</Switch>) : <></>
                    }
                    <button className="mt-4 h-11 w-32 rounded-md bg-logo-blue text-xl text-white" onClick={handleSubmit}>Submit</button>
				</div>
			),
		},
		{
			...
			content: (
				<div className="flex w-full flex-col items-start justify-start">
					...
					<Switch
						...
						className={`${
							userData.settings.privacy.personalization ? `bg-edit-green` : `bg-delete-red`
						} relative inline-flex h-8 w-16 items-center rounded-full transition`}
					>
						<span
							className={`${userData.settings.privacy.personalization ? "translate-x-9" : "translate-x-1"}
	...
	return (
		<div className="absolute flex h-full w-full flex-col">
			<SideNav />
			{ Object.keys(projMatchUser).length !== 0 && projMatchUser.userDat.profileBanner !== "" ? (
				<img
			...
			<div className="absolute left-[14%] top-[10%] z-[-1] flex h-[20%] w-[70%] flex-col">
				<div id="pfp-name" className="flex h-full w-full flex-row ">
					{Object.keys(projMatchUser).length !== 0 && projMatchUser.userDat.profilePic !== "" ? (
						<img
			...
			<div className="absolute top-[33%] z-[10] flex h-fit w-screen flex-col items-center">
				...
				<div id="main-body" className="relative flex h-fit w-[72%] flex-col">
					<div
						id="tabs-bar"
						className="flex h-[3rem] w-full flex-row items-center justify-start"
					>
						{tabs.map((tab) => (
							<button
								id={tab.id}
					...
					<div id="tabs-content" className="relative mt-4 h-[95%] w-full">
						{tabs.map((tab) => (
							<div
								id={tab.id}
...
function Popup(props) {
	return props.trigger ? (
		<div className="fixed top-0 left-0 z-[2000] flex h-full w-full flex-col items-center justify-center bg-[#000000dd]">
			<div className="flex h-[50%] w-[50%] flex-col items-center justify-center bg-white">
				{props.children}
				<button
					className="mt-5 mb-20 h-11 w-[30%] rounded-lg bg-logo-blue text-xl text-white"
